...
from add_padding import make_collate_various_size, get_global_max_shape
import random
import pickle
...
selected_letters = ['A', 'J','Z']  # example letters to keep
num_classes = len(selected_letters)

selected_letter_idxs = [class_to_idx[l] for l in selected_letters]

filtered_train_data = [pair for pair in train_data if pair[1] in selected_letter_idxs]
filtered_train_data = [
    (fpath, selected_letter_idxs.index(label)) for (fpath, label) in filtered_train_data
]
...
for train_index, test_index in kf.split(filtered_train_data):
    train_data_folds.append([filtered_train_data[i] for i in train_index])
    test_data_folds.append([filtered_train_data[i] for i in test_index])
...
class KFoldAcousticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f"Loaded sample {file_path} shape: {arr.shape}")
        # if arr.shape[0] != 4 or arr.shape[1] != 600:
        #     print(f"Unexpected shape in {file_path}: {arr.shape}")
        ...
        # arr = arr.view(4, -1)  # shape (4, L) —> use for 1D CNN
        # arr = arr.unsqueeze(1)  # add height dimension of 1
...
if __name__ == "__main__":
    # # make sure everything in dataset is same size (padded to largest)
    # global_max_height, global_max_time = get_global_max_shape(filtered_train_data)
    ...
    # Get padding dimensions *after* defining trainset

    all_fold_data = train_data_folds[fold] + test_data_folds[fold]
    global_max_height, global_max_time = get_global_max_shape(all_fold_data)

    # global_max_height, global_max_time = get_global_max_shape(train_data_folds[fold])
    collate_fn = make_collate_various_size(global_max_height, global_max_time)
    ...
    net = Acoustic2DCNN(num_classes=num_classes)  # create an instance of the Acoustic2DCNN class, passing in the number of classes
    ...
    correct = 0
    total = 0
    correct_pred = {classname: 0 for classname in selected_letters}
    total_pred = {classname: 0 for classname in selected_letters}
    ...
    with torch.no_grad():  # since we're not training, we don't need to calculate the gradients for our outputs
        for data in testloader:
            ...
            for label, prediction in zip(labels, predicted): #loop thru true labels and predicted labels for each image w/in current batch
                print(f"True: {selected_letters[label]}, Predicted: {selected_letters[prediction]}")
                if label == prediction:
                    correct_pred[selected_letters[label]] += 1 #if prediction correct increment count in correct_pred
                total_pred[selected_letters[label]] += 1 #increments total count, regardless of whether prediction was correct or not2qq

    print(f'Accuracy of the network on the {total} test images: {100 * correct // total} %')

    for classname in selected_letters:
    ...
    # save_cm_figure(ground_truth, predictions, f'cms/acoustic_cnn_cm_{fold}.png', accuracy, selected_letters)
