...
def collate_various_size(batch):
    ...
    filenames = [x[2] for x in batch]  # Get filenames
    ...
    return (data_arr, data_imu), target, filenames
...
class CNNDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        filename = self.data[index][3]  # Get filename
        ...
        return (padded_input_fn, padded_imu), output_arr, filename  # Return filename too
...
def read_from_folder(session_num, data_path, is_train=False):
    ...
    for i in range(0, len(file_echo_diff_list)):
        ...
        filename = gt[gnd].split(';')[3]  # Get the actual label from ground truth file
        truth = filename.split('_')[-1].split('.')[0].split('(')[0] #black from acoustic_diff_black(oo).npy
...
def test_model(model, test_loader, device, case_name=""):
    ...
    with torch.no_grad():
        for i, (input_arr_raw, target) in enumerate(test_loader):
            ...
            filenames.extend(target)  # Store original filenames

    test_acc = 100 * test_correct / test_total
    
    # Create results DataFrame
    results_df = pd.DataFrame({
        "Sign": [x.split('_')[-1].split('.')[0] for x in filenames],  # Get full sign name (e.g., "black(oo)")
        "True Label": [label_dic_reverse[x] for x in true_labels],  # Convert numeric labels back to strings
        "Predicted Label": [label_dic_reverse[x] for x in predictions],  # Convert numeric labels back to strings
